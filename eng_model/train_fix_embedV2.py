...
tf.flags.DEFINE_integer("dev_batch_size", 1024, "Batch Size (default: 64)")
...
with open("config_fix_embed.yml", 'r') as ymlfile:
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def batch_dev_step(x_dev, y_dev,  writer=None):
            """ 
            Evaluates model on a dev set
            """
            ...
            for batch in batches:
                ...
                total_correct_count = total_correct_count + (accuracy*len(y_batch))
                #if writer:
                #    writer.add_summary(summaries, step)
           
            net_accuracy = total_correct_count / len(y_dev)
            time_str = datetime.datetime.now().isoformat()
            print("{}: total correct {}, y_dev len {}, acc {:g}".format(time_str, total_correct_count, len(y_dev), net_accuracy))
